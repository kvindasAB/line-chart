describe("legend", () => {
    ...
    beforeEach(inject((pepito) => {
        ...
        return innerScope = element.childByClass("chart").aElement.isolateScope();
    }));
    ...
    it("should update the outer options when a series is hidden or shown", () => {
        ...
        return expect(spy.callCount).to.equal(2);
    });
    ...
    it("should create a clipping path for legend items", () => {
        ...
        return expect(clip.innerHTML()).to.equal("<circle r=\"8\"></circle>");
    });
    ...
    it("should create legend elements", inject((fakeMouse) => {
        ...
        expect(l_0.childNodes[0].getAttribute("fill")).to.equal("#4682b4");  //4682b4')
        expect(l_0.childNodes[1].getAttribute("clip-path")).to.equal("url(#legend-clip)");  //legend-clip)')

        function fn(cl) {
            return element.childByClass(cl).getStyle("display");
        }
        ...
        return expect(element.childrenByClass("legendItem")[0].getAttribute("opacity")).to.equal("1");
    }));
    ...
    it("should be able to hide a series at startup", () => {
        ...
        expect(fn("lineGroup series_0")).to.equal("none");
        expect(fn("dotGroup series_0")).to.equal("none");
        return expect(element.childrenByClass("legendItem")[0].getAttribute("opacity")).to.equal("0.2");
    });

    return describe("layout computation", () => {
        var dim, n3utils;
        n3utils = null;
        dim = {
            top: 20,
        ...
        it("should return an empty array when no legend (what is that even needed ?)", () => {
            ...
            return expect(n3utils.getLegendItemsWidths(svg, "y")).to.eql([]);
        });
        ...
        it("should compute legend", () => {
            ...
            sinon.stub(n3utils, "getLegendItemsWidths", (svg, axis) => [51, 57]);
            ...
            return expect(n3utils.computeLegendLayout({}, series, dim)).to.eql([[0, 61], [434, 495]]);
        });

        return it("should compute for left and right series", () => {
            var series;
            ...
            return expect(n3utils.computeLegendLayout({}, series, dim)).to.eql([[0, 109], [288, 403]]);
        });
    });
});
